        ...
        <v-row>
            <v-col cols="12" md="4">
                <v-card class="pa-4" outlined>
                    <v-icon size="48" colors="primiary" class="mb-4">mdi-book-print</v-icon>
                    <h2 class="subtitle-1 font-weight-bold mb-4">선호 책 예측</h2>
                    <v-select
                        v-model="gender"
                        :items="genderOptions"
                        label="성별"
                    ...
                    <v-btn block color="success" class="mt-2" @click="predictPreferredBook" :loading="loading">
                        <v-icon left>mdi-book-print></v-icon>
                        선호 도서 예측
                    </v-btn>
                </v-card>
            </v-col>
                ...
                <v-card class="pa-4" outlined>
                    <div v-if="trainResult || predictionResult || error">
                        ...
                        <div v-if="predictionResult">
                            <p><strong>선호 도서:</strong>{{ predictionResult.predicted_preferred_book }}</p>
...
export default {
    ...
    methods: {
        async trainModel() {
            this.executePostResponse("/train-preferred-book");
        },
        async predictPreferredBook() {
            const data = {
                gender: this.gender,
                birth_year: this.birthYear,
            };
            this.executePostResponse("/predict-preferred-book", data, ["predicted_preferred_book", "prefer_probability"]);
        },
        async executePostResponse(url, data = {}, resultKeys = null) {
            this.resetState();
            this.loading = true;
            try {
